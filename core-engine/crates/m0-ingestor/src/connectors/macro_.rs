...
#[derive(Debug, Clone)]
pub struct MacroConnector {
    pub market_id: String,
}

#[async_trait]
impl Connector for MacroConnector {
    async fn run(&self, producer: Producer) -> anyhow::Result<()> {
        info!(market_id = %self.market_id, "macro connector started (simulated)");
        loop {
            ...
            let ev = RawEvent {
                ...
                payload: json!({ "kind": "macro_signal", "signal": signal }),
                dedupe_key: format!("macro:{}:{}", self.market_id, (signal * 1_000_000.0) as i64),
            };
